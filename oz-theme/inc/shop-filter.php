...
function oz_shop_filter_productlijnen() {
	/* Beton Ciré has three variants — All-in-One (= Kant & Klaar), Easyline
	   and Original — which live in separate product_cat slugs in WC but are
	   the same product line for the shopper. Roll the kant-en-klaar slug
	   into the beton-cire entry so it doesn't show up as a separate filter
	   row that's logically a subset of Beton Ciré. */
	return [
		'beton-cire'      => [ 'label' => 'Beton Ciré',       'cats' => [ 'beton-cire', 'all-in-one', 'easyline', 'original', 'kant-en-klaar' ] ],
...
function oz_shop_filter_render_sidebar() {
	...
	<form class="oz-shop-filter" method="get" action="<?php echo esc_url( oz_shop_filter_url( [ 'lijn' => [], 'kleur' => [], 'prijs_min' => null, 'prijs_max' => null, 'q' => '' ] ) ); ?>" data-oz-shop-filter>
		...
		<?php /* Prijs-fieldset bewust weggelaten — binnen een productlijn liggen
			      alle prijzen vrijwel vast (Lavasteen €235, microcement vlak, etc),
			      dus de filter dupliceerde productlijn zonder extra narrowing. URL
			      param ?prijs=min-max blijft server-side gehonoreerd voor oude
			      bookmarks; alleen de UI is verwijderd. */ ?>
