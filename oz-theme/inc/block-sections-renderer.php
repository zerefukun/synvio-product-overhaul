...
function oz_render_block_sections( $content ) {
	...
	foreach ( $blocks as $i => $block ) {
		...
		} elseif ( strpos( ltrim( $rendered ), '<section class="oz-hp-hero"' ) === 0 ) {
			/* In-content homepage-style hero (core/html block dat begint met
			   <section class="oz-hp-hero">): full-bleed renderen, geen
			   .oz-container wrapper, geen extra <section class="oz-section">.
			   Plus poster-vibe (V4): data-hero-label + .oz-hp-hero-marker
			   injecteren zodat de 13 in-content hero pages dezelfde watermark +
			   verticale marker krijgen als de template-rendered pages. */
			$hero_data = function_exists( 'oz_get_ruimte_hero_data' )
				? oz_get_ruimte_hero_data()
				: array( 'label' => '', 'marker' => '' );

			if ( $hero_data['label'] !== '' ) {
				/* Voeg data-hero-label="..." toe aan de <section class="oz-hp-hero">
				   opening-tag — alleen de eerste match (de hero zelf). */
				$rendered = preg_replace(
					'/<section\s+class="oz-hp-hero"\s*(?![^>]*data-hero-label)/',
					'<section class="oz-hp-hero" data-hero-label="' . esc_attr( $hero_data['label'] ) . '" ',
					$rendered,
					1
				);
			}
			if ( $hero_data['marker'] !== '' ) {
				/* Marker-span direct na de <img class="oz-hp-hero-bg" …> tag injecteren. */
				$marker_html = '<span class="oz-hp-hero-marker">' . esc_html( $hero_data['marker'] ) . '</span>';
				$rendered = preg_replace(
					'/(<img\s+class="oz-hp-hero-bg"[^>]*>)/',
					'$1' . $marker_html,
					$rendered,
					1
				);
